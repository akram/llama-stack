...
class PageIndexRagToolRuntimeImpl(ToolGroupsProtocolPrivate, ToolRuntime):
    """Vectorless RAG implementation using PageIndex for reasoning-based retrieval."""
    ...
    def __init__(
        ...
        self.pageindex_client = PageIndexClient(config.pageindex_config)
    ...
    async def insert(
        ...
        for doc in documents:
            try:
                ...
                # Build PageIndex tree (no chunking, no embeddings)
                try:
                    await self.pageindex_client.build_index(
                        store_id=vector_store_id,
                        file_id=created_file.id,
                        file_data=file_data,
                        metadata={**doc.metadata, "filename": filename},
                    )
                except Exception as e:
                    log.error(
    ...
    async def _query_pageindex_store(
        ...
        try:
            pageindex_results = await self.pageindex_client.query(
                store_id=store_id,
                query=query,
        ...
        return ListToolDefsResponse(
            data=[
                ToolDef(
                    ...
                    description="Insert documents into memory using PageIndex (vectorless, tree-based)",
                ),
                ToolDef(
                    name="knowledge_search",
                    description="Search for information using reasoning-based retrieval (PageIndex).",
                    input_schema={
                        "type": "object",
                        "properties": {
                            "query": {
